...
# PurpleBee - AI-Powered Productivity Dashboard · v1.18.0
...
- **Project References & Task References, app-wide** — Projects now have their own Attachments & Links, separate from any individual task's: a "Project References" section is available when creating a project, editing a project, and on the Project Detail page (persists immediately via the same `attachments`/`links` JSONB pattern as tasks, now on `projects` too). All four surfaces (Create/Edit Project, Project Detail, Create/Edit Task) share one implementation (`ReferencesSection`) with 15 categorized reference types (Discovery Meeting, Wireframe, Requirements, Scope Doc, Figma, GitHub, Notion, Google Doc/Drive, SharePoint, OneDrive, Loom, YouTube, Vimeo, Link, Other), auto-detected from the URL or set manually; when a task belongs to a project, a read-only "From project: …" panel surfaces that project's references directly in the task without duplicating any files. Uploads now store real `data:` URLs (capped at 5MB) with uploader name/timestamp recorded per file, fixing a prior bug where attachments used session-only `blob:` URLs that broke after a page reload
