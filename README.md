...
- **Drag-to-reorder project grid** — hover any project card to reveal the grip handle (⠿) on the left edge; initiate the drag from the grip handle (the handle is the HTML drag source so inner buttons do not interfere); drag over another card to reorder; the dragged card dims and shrinks, the drop target highlights with a purple ring and lifts slightly; order is persisted to `localStorage` keyed by user ID and survives page refresh; new projects append to the end; deleted projects are pruned automatically
