...
- **Sort projects** — a "Sort by" dropdown next to the search bar offers Custom Order (the drag order above, default), Date Created, Date Updated, or Name, each with an ascending/descending toggle button; drag-to-reorder is only active while Custom Order is selected (dragging while sorted by date/name is disabled — it would silently reorder a hidden list without any visible effect); the choice persists to `localStorage` keyed by user ID
